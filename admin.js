...
function adminDeliveryText(minutes){minutes=Math.max(0,Number(minutes)||0);const h=Math.floor(minutes/60),m=minutes%60;return h?(m?`${h}h ${m}m`:`${h}h`):`${m}m`}
async function loadVehicleDeliverySetting(){
  if(!adminUser||!$('#adminVehicleDeliveryHours'))return;
  try{const d=await adminVehicleApi('admin_get_delivery');$('#adminVehicleDeliveryHours').value=(Number(d.deliveryMinutes||180)/60).toFixed(Number(d.deliveryMinutes||180)%60?2:0);$('#adminVehicleDeliveryStatus').textContent='Current: '+adminDeliveryText(d.deliveryMinutes)}catch(e){$('#adminVehicleDeliveryStatus').textContent='Could not load: '+e.message}
}
if($('#saveVehicleDeliveryDelayBtn'))$('#saveVehicleDeliveryDelayBtn').onclick=async()=>{
  const st=$('#adminVehicleDeliveryStatus');try{const hours=Number($('#adminVehicleDeliveryHours').value);if(!Number.isFinite(hours)||hours<0||hours>168)throw new Error('Enter a time between 0 and 168 hours.');const mins=Math.round(hours*60);st.textContent='Saving...';const d=await adminVehicleApi('admin_set_delivery',{deliveryMinutes:mins});st.textContent='Saved: '+adminDeliveryText(d.deliveryMinutes);showNotice('Vehicle delivery time updated.','Vehicle Shop','success')}catch(e){st.textContent='Failed: '+e.message;showNotice(e.message,'Vehicle Shop','danger')}
};
...
async function loadAdminVehiclePurchases(){const box=$('#adminVehiclePurchasesList');if(!box||!adminUser)return;box.innerHTML='<p>Loading purchases...</p>';try{const d=await adminVehicleApi('admin_purchases');const rows=d.purchases||[];box.innerHTML=rows.length?`<div class="redeem-claims-table-wrap"><table class="redeem-claims-table"><thead><tr><th>Player</th><th>Vehicle</th><th>eCoin</th><th>Status</th><th>Vehicle ID</th><th>Purchased</th></tr></thead><tbody>${rows.map(r=>`<tr><td><strong>${esc(r.server_username)}</strong><small>UID: ${Number(r.server_uid)}</small></td><td>${esc(r.vehicle_name)}<small>Model ${Number(r.modelid)}</small></td><td>${Number(r.ecoin_price).toLocaleString()}</td><td><span class="status-pill ${r.delivery_status==='delivered'?'accepted':r.delivery_status==='failed'?'rejected':'pending'}">${esc(String(r.delivery_status||'pending').toUpperCase())}</span><small>${r.delivery_status==='pending'&&r.deliver_at?'ETA: '+fmt(r.deliver_at):r.delivered_at?'Delivered: '+fmt(r.delivered_at):''}</small></td><td>${r.vehicle_id?Number(r.vehicle_id):'—'}</td><td>${fmt(r.purchased_at)}</td></tr>`).join('')}</tbody></table></div>`:'<p class="muted">No vehicle purchases yet.</p>'}catch(e){box.innerHTML='<p>Could not load purchases: '+esc(e.message)+'</p>'}}
...
adminAuth.onAuthStateChanged(user=>{if(user)setTimeout(()=>{loadVehicleDeliverySetting();loadAdminVehicleShop();loadAdminVehiclePurchases()},450)});
